  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 tracking-tight uppercase">Audit & Financial Intelligence</h2>
          <p className="text-slate-500 text-sm font-medium">Global transaction manifest and fiscal performance metrics.</p>
        </div>
        <div className="bg-white px-6 py-3 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center text-right">
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Enterprise Net Yield</span>
      ...
      <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
        <h3 className="font-black text-slate-800 mb-6 flex items-center gap-3 uppercase tracking-tight text-sm">
          ...
          Liquidity Flow (Real-Time Timeline)
        </h3>
        <div className="h-64">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Immutable settlement records for all nodes.</p>
          </div>
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-100">
                <th className="px-8 py-6 font-black text-slate-400 uppercase tracking-widest text-[10px]">Reference & Timestamp</th>
                <th className="px-6 py-6 font-black text-slate-400 uppercase tracking-widest text-[10px]">Facility</th>
                <th className="px-6 py-6 font-black text-slate-400 uppercase tracking-widest text-[10px]">Method</th>
                <th className="px-6 py-6 font-black text-slate-400 uppercase tracking-widest text-[10px]">Settlement</th>
                ...
                <tr key={t.id} className={`hover:bg-slate-50/50 transition-colors group ${t.isVoid ? 'opacity-40 grayscale' : ''}`}>
                  <td className="px-8 py-5">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-tighter mt-1">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                       <i className="fas fa-store text-slate-300 text-[10px]"></i>
                       <span className="text-xs font-black text-slate-600 truncate max-w-[120px]">
                         {branches.find(b => b.id === t.branchId)?.name || 'HQ Branch'}
                       </span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    {t.isVoid ? (
                      <div className="flex flex-col">
                        <span className="bg-rose-50 text-rose-600 border border-rose-100 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest w-fit shadow-sm">
                          VOIDED
                        </span>
                        <span className="text-[8px] text-rose-400 mt-1.5 italic font-bold leading-tight max-w-[140px] truncate" title={t.voidReason}>
                          {t.voidReason}
                        </span>
                      </div>
                    ) : (
                      <span className="bg-emerald-50 text-emerald-600 border border-emerald-100 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest w-fit shadow-sm">
                        SETTLED
                      </span>
                    )}
                  </td>
              ...
              {transactions.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-32 text-center text-slate-300 italic">
                    <i className="fas fa-receipt text-5xl mb-4 block opacity-10"></i>
                    <p className="text-sm font-black uppercase tracking-widest">No financial data provisioned</p>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {isVoiding && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[60] p-4">
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-10 animate-slideUp">
            <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase mb-2">Record Invalidation</h3>
            <p className="text-sm text-slate-500 font-medium mb-8">Authorizing the void of this settlement requires a mandatory audit statement.</p>
            
            <textarea 
              autoFocus
              className="w-full h-32 p-6 bg-slate-50 border border-slate-200 rounded-3xl outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 text-sm font-medium mb-8 resize-none"
              placeholder="Provide forensic justification..."
              value={voidReason}
            ...
            <div className="flex gap-4">
              <button 
                onClick={() => { setIsVoiding(null); setVoidReason(''); }}
                className="flex-1 py-4 bg-slate-100 rounded-2xl font-black text-slate-500 uppercase tracking-widest text-[10px] hover:bg-slate-200 transition-colors"
              >
                Abort
              </button>
              <button 
                onClick={() => handleVoid(isVoiding)}
                className="flex-[2] py-4 bg-rose-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-rose-700 shadow-xl shadow-rose-500/20 transition-all active:scale-95"
              >
                Authorize Void
              </button>
            </div>
